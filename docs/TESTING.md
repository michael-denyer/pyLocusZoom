...
- `tests/figure_probes.py` — one probe object per backend (`PROBES`), translating panel count, tick labels, legend corner and edge colours, threshold lines, rectangles, region highlights, point alpha, font sizes, marker symbols and hover into one vocabulary; the only place in the suite that knows matplotlib's, plotly's or bokeh's figure internals. `standalone_html` and `json_payload` exist only for the interactive backends
