...
- A CI `examples` job downloads the canine recombination maps, runs `examples/generate_example_plots.py` into a temporary directory and executes `examples/getting_started.ipynb` headless. `scripts/execute_notebook.py` runs the notebook with nbclient and can save its outputs; `nbclient` and `ipykernel` join the `dev` dependency group.
- USER_GUIDE shows the stacked Manhattan and QQ plot (`plot_manhattan_qq_stacked`) and the canine Manhattan and QQ plot, both generated but unlinked before, and says where the plotly and bokeh HTML exports are and how to open them.
...
- Contributor docs: the CONTRIBUTING quick start runs `uv sync --extra dev`, since plain `uv sync` installs no test tools and the suite failed on a missing `hypothesis`. ARCHITECTURE no longer says an unknown species raises (it becomes an Ensembl-only record) and states that an `ld.lead_pos` matching no SNP only logs; its component diagram and the CODEMAP overview, regional sequence and backend class diagrams are redrawn from the import graph and the `render_figure` flow. Dangling references (`panel_y`, `_AssociationInput`, two anchors) and the claim that `load_bed` and `load_caviar` are bespoke loaders are corrected. TESTING gives the 22 unowned test files an owner and resolves whether panel classes are test seams.
...
- The example generator writes plotly exports with `include_plotlyjs="cdn"`. Each file embedded the 4.9 MB plotly.js bundle, so `examples/plotly/` was 60 MB and every regeneration committed another 60 MB; it is now about 4.5 MB. USER_GUIDE notes the same option for `write_html`.
- The example generator fails on any `UserWarning` and fetches the canine recombination maps before plotting. On a cold cache it exited 0 and wrote the README hero image without its recombination overlay, which `scripts/example_diff.sh --keep` would have accepted. `example_diff.py` also runs the generator with `uv run --frozen`.
- The getting-started notebook shows each matplotlib figure once instead of twice, draws the lead diamond in its first plot, seeds every cell that draws random numbers and saves cell 26's files into a temporary directory. Its HLA gene table now matches Ensembl GRCh38: HLA-B and HLA-C had each other's coordinates, HLA-DQB1 had HLA-DRB1's, and three strands were wrong.
- USER_GUIDE states which plots have hover tooltips. Only the regional, eQTL and fine-mapping panels have per-SNP tooltips; the Miami entry claimed hover in both interactive backends, but bokeh draws none for genome-wide, QQ, colocalization, LD heatmap, PheWAS or forest plots, and plotly shows only the raw axis values for them.
...
- **The managed canine recombination maps download again, and they are the sex-averaged maps.** The Campbell et al. archive ships average, female and male maps for each chromosome. Since 4.0.0 the download matched all three: installs that predate the duplicate check kept a sex-specific map, and every fresh install since has failed with `DataDownloadError: Duplicate chromosome map`. The download now stages only `chr*_average_*.txt`. An existing managed cache may hold a sex-specific map; refresh it with `download_canine_recombination_maps(force=True)`.
...
- **A config model rejects a field it does not declare.** `LDConfig(ld_column="R2")`, `DisplayConfig(lable_top_n=3)` or a 4.x `PanelInputs(eqtl_df=...)` built a model with the keyword silently dropped, so the plot drew without the option. Every config model now raises `ValidationError` naming the field. Migration: correct the name; for `PanelInputs`, run the migration script.
...
- **A lead outside the region raises.** `plot()` and `plot_stacked()` accepted an `ld.lead_pos` or `lead_positions` entry outside `start`-`end` and drew the lead off the axis without a word. The bounds are inclusive, and with liftover both are source-build coordinates. Migration: pass a lead inside the region, or omit it to auto-detect the strongest in-region SNP.
...
- **Recombination lookups raise instead of returning a status.** `RecombStatus`, `RecombResult` and `recomb_for_region` are removed. `get_recombination_rate_for_region` is the one lookup: with no `data_dir` it now downloads the managed maps itself, and every reason there is no frame is a `PyLocusZoomError`. Migration: call `get_recombination_rate_for_region` and catch `PyLocusZoomError`, or the type for the old status: `DOWNLOAD_FAILED` is `DataDownloadError`, `NO_MAPS_FOR_SPECIES` and `NO_MAP_FOR_CHROMOSOME` are the new `RecombinationMapNotFound`, `LIFTOVER_UNAVAILABLE` is `OptionalDependencyMissing` and `BUILD_UNAVAILABLE` is `ValidationError`.
- **`RecombinationMapNotFound` replaces `FileNotFoundError` from `load_recombination_map`.** It subclasses `FileNotFoundError`, so existing `except FileNotFoundError` still works. A species with no built-in maps and no `data_dir` raises it at once instead of reading whatever the managed cache holds.
- **A map set that cannot be written is a `DataDownloadError`.** `ensure_recomb_maps` and `download_canine_recombination_maps` raised a bare `OSError` for a read-only or unusable cache. Migration: catch `DataDownloadError`.
- **Logging is off until `enable_logging()`, and `LocusZoomPlotter(log_level=...)` is removed.** The package used to log at INFO from import, and the plotter's constructor turned logging back on after `disable_logging()`. Import now calls loguru's `logger.disable("pylocuszoom")` and adds or removes no handler, which replaces the 4.2.1 change that swapped loguru's default sink for a filtered copy. Migration: drop `log_level=None`; replace `log_level="DEBUG"` with `enable_logging("DEBUG")`. The package's records also reach any unfiltered sink the application added, loguru's default stderr sink included.
- **LD functions take `species` as a required keyword.** `calculate_ld`, `calculate_pairwise_ld`, `build_ld_command` and `build_pairwise_ld_command` defaulted to `"canine"`, so a human caller ran `plink --dog` and had chromosomes 23-26 read as dog autosomes without an error. `species=None` still means PLINK's default (human) set. Migration: pass `species="canine"` to keep the old command, and pass `threads` and `metric` by keyword.
- **PLINK not found raises `PlinkError`.** It raised `FileNotFoundError`, and a PLINK path that could not be executed escaped as a raw `OSError`; both are now `PlinkError`. Migration: catch `PlinkError`.
- **Genome-build facts moved to `pylocuszoom.genome_build`.** One `GenomeBuild` record per build, resolved by `resolve_build`, holds the synonyms, the UCSC gene genome, the liftover chains and the UCSC chromosome renames. Removed: `utils.ASSEMBLY_SYNONYMS`, `reference_genes.UCSC_BUILDS`, `reference_genes.ucsc_genome_for_build`, `ucsc.UCSC_ASSEMBLY_NAMES`, `RecombSource.liftover_chains`, `recombination.CANFAM3_TO_CANFAM4_CHAIN_URL` and `Species.chain_chrom_aliases`. Migration: `utils.assembly_token` is `genome_build.assembly_token`; `ucsc_genome_for_build(b)` is `resolve_build(b).ucsc_genome` (None for an unknown build); chain URLs and renames are `GenomeBuild.liftover_chains` and `GenomeBuild.ucsc_chrom_renames`.
- **`liftover_region` takes `build=` instead of `species=`.** PLINK's numeric X codes are UCSC names of a build, so the renames come from the target build: pass a build name such as `"canfam4"` or a `GenomeBuild`. It no longer warns for a chromosome the chain lacks; `RegionLiftResult.chain_has_chrom` says so.
- **One chain loader.** `recombination.get_chain_dir`, `get_chain_file_path`, `download_liftover_chain` and `liftover_recombination_map` are removed, with the private `_liftover.liftover_positions` and `chain_chrom`. Registered chains download and load through `_liftover.chain_lifter` and the cached `load_chain`. Migration: lift a frame with `liftover_region(frame, chrom=..., lifter=..., pos_col="pos")`, and load a chain file with `LiftoverConfig(chain_path=...).resolve()`.
- **`ucsc.fetch_track_frames` and `ucsc.ucsc_source` take a build.** Pass a `GenomeBuild` or any spelling of a build UCSC serves genes for (`"canFam3"`, `"CanFam3.1"`); any other build raises `ValidationError`.
...
- **`plot_stacked()` takes `liftover=`, through the same code path as `plot()`.** Every frame is lifted, the source region is selected by `ColumnConfig.chrom_col` (or by position alone for `chrom_col=None`), and the window spans the lifted SNPs of all panels with the requested margins. Both methods validate their config in source-build coordinates before lifting, and a lead that does not lift while `ld_reference_file` needs it raises `ValidationError` instead of warning that the lead is auto-detected and then raising.
- **LD colouring that cannot be computed is a `UserWarning`, like a skipped gene track or recombination overlay.** An empty PLINK output and PLINK output naming a variant twice each draw the panel without LD colouring and warn once at the caller's `plot()` or `plot_stacked()` line. They used to reach only the log. Duplicate ids raise the new `LDUnavailableError`, a `ValidationError`, from `calculate_ld` and `calculate_pairwise_ld`.
- **A failed recombination lookup is retried on the next plot.** The plotter caches frames per region, build and lifter, but no longer memoises a failure.
- **Managed map sets install without a missing-directory window.** An absent set arrives in one directory rename; over an existing set each map file is replaced in place. Readers never find the directory missing, and a writer that loses a race to another succeeds.
- **Plotly and bokeh tooltips show the same fields in the same formats.** Formats came from substrings of each column's display name, and plotly also took the first column to be the SNP id. The fine-mapping and eQTL panels have none, so plotly showed their position bold, unlabelled and unformatted while bokeh showed it labelled and comma-grouped. Each column's format now comes from its role, and every plotly field carries its label, so the association tooltip's bold SNP line reads `SNP: rs123`.
- **Plotly panel titles are annotations above their own panel.** The first panel of a one-column plotly figure was titled through the layout title, the slot the figure title uses, so a figure with both lost the panel title. Every panel title is now a bold annotation just above its panel, as grid titles already were, so a titled one-column plotly figure (a Manhattan, QQ, PheWAS, forest, colocalization or LD heatmap plot, or the top of a stacked Manhattan) shows its title bold and centred over the panel instead of at the figure's top left. Grid titles sit 4 px above their panel instead of 5% of its height.
- **`_render_regional` is four named steps.** Genes and recombination (`_resolve_annotations`), the fine-mapping, eQTL and gene panels (`panels.optional_panels`), LD colouring and the association panels (`_association_panels`), and the LD heatmap (`panels.ld_heatmap_panels`). The optional panels still validate their frames before PLINK runs; a recombination lookup now happens before that validation instead of after it.
- **Every panel is built through its own constructor, and no `draw` checks which columns a frame has.** `AssociationPanel.from_input`, `ColocPanel.from_frames` and `LDHeatmapPanel.from_matrix` join the existing constructors; the colocalization merge and lead resolution move from `coloc_plotter.py` into `panels/coloc.py`, and PheWAS effect markers are assigned when the panel is built. The Manhattan panel is declared once (`manhattan_spec`, `categorical_spec`, `render_manhattan_panel` and `render_qq_panel` are gone), and both LD heatmap panels draw through `composition.draw_ld_heatmap`. These are internal modules; rendered output is unchanged.
- **Internal modules renamed or split.** `_ld_plotting.py` is `_ld_enrichment.py`; the drawing constants and `add_significance_line` move from `_plotter_utils.py` to `panels/_shared.py`.
...
- `ColocConfig`, exported from `pylocuszoom`.
- `backends.hover.HoverData` and `HoverRole`, which carry each tooltip column's role to the backend.
...
- **Concurrent map downloads no longer leak a hidden `.previous-*` directory or report a failed download after another writer succeeded.**
- **A Manhattan plot of one SNP, or of SNPs all at one position, has an x axis with width.** The axis was padded by 1% of the data's span, which is zero there, so plotly and bokeh got an empty range and matplotlib warned. It is now padded by half the chromosome gap.
- **Two processes replacing an older release's symlinked map cache no longer fail.** The writer that found the symlink already removed raised `FileNotFoundError`, or `PermissionError` once the other had published, so its plot drew without the recombination overlay.
...
- **`import pylocuszoom` leaves the host application's loguru output alone.** The import removed loguru's default stderr sink, so the application's own `logger.info` calls printed nothing afterwards. Import now adds and removes no handler and only disables the package's records (see the logging entry under Breaking). After `enable_logging()`, a record prints twice while loguru's default stderr sink is still installed: once through that sink and once through the sink `enable_logging` adds.
...
- **`CITATION.cff` and `.zenodo.json`.** GitHub shows a "Cite this repository" button, and every release from this one on is archived on Zenodo with its own DOI. The README has a Citation section with a BibTeX entry, and the release checklist in `docs/DEVELOPMENT.md` keeps the `CITATION.cff` version in step with `pyproject.toml`.
...
- **`loaders.py` is a `loaders/` package, one module per family (breaking for direct submodule imports).** The file was 929 lines, of which only about 400 are code; the rest is the per-format documentation and the declarative `LoaderSpec` tables, which belong beside the loaders they describe. `pylocuszoom.loaders` is now a package: `_engine.py` holds `LoaderSpec`, `_load_tabular` and the output-column token mechanism, and `gwas.py`, `eqtl.py`, `finemapping.py` and `annotation.py` hold one family each, with the shared-argument prose that used to sit in the module docstring now in the family module it describes. Every public loader is re-exported, so `from pylocuszoom.loaders import load_gwas` and `pylocuszoom.load_gwas` are unchanged. Only the private names move: `pylocuszoom.loaders._detect_format` is now `pylocuszoom.loaders.gwas._detect_format`, and `LoaderSpec` and `_load_tabular` are in `pylocuszoom.loaders._engine`. No loader logic changed, and loaded output is unchanged.
