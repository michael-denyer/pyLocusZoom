...
- **`get_genes_for_build()`**, the build-aware entry point that picks the source, alongside `ucsc_genome_for_build()`, `fetch_genes_from_ucsc()`, `fetch_exons_from_ucsc()`, `get_genes_for_region_ucsc()` and `clear_ucsc_cache()`. `UCSCAPIError` mirrors `EnsemblAPIError`, keeping a service failure distinguishable from a region that genuinely has no genes.
